...
class CUDACallback(Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        # Atualizado: usar strategy.root_device.index em vez de root_gpu
        device = trainer.strategy.root_device.index if hasattr(trainer.strategy, 'root_device') else 0
        torch.cuda.reset_peak_memory_stats(device)
        torch.cuda.synchronize(device)
        self.start_time = time.time()

    def on_train_epoch_end(self, trainer, pl_module):  # Removido 'outputs'
        device = trainer.strategy.root_device.index if hasattr(trainer.strategy, 'root_device') else 0
        torch.cuda.synchronize(device)
...
class SetupCallback(Callback):
    ...
    def on_fit_start(self, trainer, pl_module):  # Mudado de on_pretrain_routine_start
        if trainer.global_rank == 0:
