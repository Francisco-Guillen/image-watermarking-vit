...
import torchvision
from cldm.vit_secret_decoder import ViTSecretDecoder
...
class SecretDecoder(nn.Module):
    def __init__(self, arch='CNN', act='ReLU', norm='none', resolution=256, in_channels=3, secret_len=100):
        ...
        elif arch in ['vit', 'vit_small', 'vit_small_patch16']:
            self.decoder = ViTSecretDecoder(
                msg_len=secret_len,
                img_size=resolution,
                token_drop=0.0,
                pretrained=True
    ...
    def forward(self, image):
        if image.shape[-2:] != (self.resolution, self.resolution):
            image = thf.interpolate(image, size=(self.resolution, self.resolution),
